...
class Backtest(object):
    ...
    def get_port(self, port_type='equal_weighted', neutral='market'):
        ...
        def equal_weight(x):
            x_weight = 1.0 * (x - x.quantile(0.8) > 0) - 1.0 * (
                    x - x.quantile(0.2) < 0)
        ...
        if port_type == 'equal_weighted':
            if neutral == 'market':
                self.alpha['weights'] = self.alpha.groupby('Date')['alpha']\
                    .transform(equal_weight)
            elif neutral == 'industry':
                self.alpha['weights'] = self.alpha.groupby(
                    ['Date', 'Industry'])['alpha'].transform(equal_weight)
            else:
                raise Exception('Not Implemented')
        elif port_type == 'alpha_weighted':
            if neutral == 'market':
                self.alpha['weights'] = self.alpha.groupby('Date')['alpha']\
                    .transform(alpha_weight)
            elif neutral == 'industry':
                self.alpha['weights'] = self.alpha.groupby(
                    ['Date', 'Industry'])['alpha'].transform(alpha_weight)
            else:
                raise Exception('Not Implemented')
        else:
            raise Exception('Not Implemented')
    ...
    def get_rtns(self, port_type='equal_weighted', neutral='market'):
        ...
        self.merged_df = pd.merge(self.alpha, self.stock_rtn, how='left',
                                  left_on=['Date', 'Ticker'],
                                  right_on=['Date', 'Ticker'])

        self.merged_df['rtn_times_weight'] = self.merged_df['Rtn1q'] * \
            self.merged_df['weights']
        self.port_rtns = self.merged_df.groupby('Date')['rtn_times_weight']\
            .sum()
    ...
    def summary_statistics(self):
        ...
        print(f'sharpe ratio {sharpe:.4f},'
              f' annual return {annual_return:.4f}, mdd {mdd:.4f}')
...
class alpha_generate(object):
    ...
    def Train(self, begin='2016-01-01', end='2017-01-01'):
        ...
        time_indicator = (self.df['Date'] >= pd.to_datetime(begin,
                                                            format='%Y-%m-%d')
        ...
        self.model.Inference(X_tensor, Y_tensor, lr=0.001,
